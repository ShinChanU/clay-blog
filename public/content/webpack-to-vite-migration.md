...
`Vite`가 이렇게 빠른 이유는,  **개발 서버를 띄울 때 번들링을 건너 뛰고, 필요한 모듈의 `네이티브 ESM` 소스 코드를 직접 제공하는 방식**으로 콜드 스타트를 매우 짧게 가져가기 때문입니다. 또한 `Vite`의 `HMR` 도 브라우저의 `네이티브 ESM` 을 활용하기 때문에 변경된 모듈만 즉각 갱신됩니다. `Webpack` 처럼 전체 종속성을 다시 번들링하지 않아도 되어 브라우저가 즉각적으로 업데이트를 반영합니다. 특히 CSS 파일의 변경도 빠르게 적용되므로 스타일 수정 작업이 한결 편리합니다. 아래 이미지를 참고하면 좋겠습니다.
...
저에게 `Vite`로의 마이그레이션은 단순히 도구를 바꾼 것보다는 더 나은 개발 환경을 추구하는 과정과 결과라는 점에서 더 유의미한 실험이였습니다.
